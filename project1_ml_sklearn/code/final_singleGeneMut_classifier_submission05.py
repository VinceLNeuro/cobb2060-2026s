...
def read_mut(fname, skiprows=0, sample_col="Tumor_Sample_Barcode", mut_type_col="Consequence"):
    """Read only columns needed for labels (much faster than full TSV)."""
    import warnings
    warnings.simplefilter(action='ignore', category=pd.errors.DtypeWarning)
    
    use = ["Hugo_Symbol", sample_col, mut_type_col]
    df = pd.read_table(fname, sep="\t", header=0, skiprows=skiprows,
        usecols=lambda c: c in use,
        dtype={"Hugo_Symbol": "string", sample_col: "string", mut_type_col: "string"},
        low_memory=False,
    )
...
ref_hugo_entrez['Entrez_Gene_Id'] = pd.to_numeric(ref_hugo_entrez['Entrez_Gene_Id'], errors='coerce')
ref_hugo_entrez = ref_hugo_entrez.dropna(subset=['Entrez_Gene_Id','Hugo_Symbol'])
ref_hugo_entrez['Entrez_Gene_Id'] = ref_hugo_entrez['Entrez_Gene_Id'].astype(np.int64)
ref_hugo_entrez = ref_hugo_entrez.drop_duplicates(subset=['Entrez_Gene_Id'], keep='first')

def sortByEntrez(data, entrez_col = 'Entrez_Gene_Id', do_sort=False):
    # dropNA and do not sort to increase speed
    if entrez_col not in data.columns:
        return data
    out = data.copy()
    out[entrez_col] = pd.to_numeric(out[entrez_col], errors='coerce')
    out = out.dropna(subset=[entrez_col])
    out[entrez_col] = out[entrez_col].astype(np.int64)
    if do_sort:
        out = out.sort_values(entrez_col, kind="mergesort", ignore_index=True)
    else:
...
for key in ls_expr_df.keys():
    df = ls_expr_df[key][0].copy()
    df_sorted = sortByEntrez(df, 'Entrez_Gene_Id', do_sort=False)
    # faster: first use the lookup table to fill, then use original hugo to fill
...
mutated_samples_by_gene = {g: set() for g in target_genes}
genes_seen_in_mut = set()
mut_samples = set()
...
for subpath in args.train_mut:
    ...
    m = read_mut(path, skiprows=skip)
    
    # unique samples present in this mutation file (-> for alignment)
    mut_samples.update(m["Tumor_Sample_Barcode"].dropna().astype(str).unique()) # adding unique items

    ### filter to keep only 1) target genes + 2) non-synonymous (substitute for the function before) ###
    m = m[m["Hugo_Symbol"].isin(target_gene_set)]
    if len(m) == 0:
        continue
    m = m[~m["Consequence"].str.contains("synonymous_variant", na=False)]

    for g, gdf in m.groupby("Hugo_Symbol", sort=False):
        genes_seen_in_mut.add(g)
        mutated_samples_by_gene[g].update(gdf["Tumor_Sample_Barcode"].dropna().astype(str).unique()) #gene-specific
...
#     merged['mutated'] = merged['mutated'].fillna(False).astype(bool)
#     # display(merged)
...
#         columns=sample_col,
#         values='mutated'
#     ).fillna(False).astype(bool) #those genes with no mutation in the samples
...
# merged_mut = pd.concat(mut_list, axis=1).fillna(False).astype(bool) #in case some genes not in all samples
...
#### Align the expression samples to mutation samples
common_samples = merged_expr.columns.intersection(pd.Index(mut_samples))
merged_expr = merged_expr.loc[:, common_samples]
print(f"Aligned expression (by samples): {merged_expr.shape}")
...
X_train_scaled = np.ascontiguousarray(X_train_scaled.astype(np.float32, copy=False)) #faster for saga opt
...
X_test_scaled  = np.ascontiguousarray(X_test_scaled.astype(np.float32, copy=False)) # faster for saga
...
for gene in target_genes:
    ...
    if gene not in genes_seen_in_mut:
    ...
    # Prepare y
    mut_set = mutated_samples_by_gene.get(gene, set()) # get corresponding samples set
    ##                                  membership test - convert to numerical for sklearn
    y = X_train_aligned.index.to_series().isin(mut_set).to_numpy(dtype=np.int32)
