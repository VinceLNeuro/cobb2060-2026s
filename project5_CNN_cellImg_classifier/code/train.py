...
def run_training(args):
    ...
    for epoch in range(args.max_epochs):
        ...
        if val_acc_perEpoch > best_val_acc:
            ...
            # Save best checkpoint during training - not final model export
            ...
            # testdataloader = torch.utils.data.DataLoader(eval_subset, batch_size=1) #one example at a time for testing
            # testbatch = next(iter(testdataloader))

            testbatch = next(iter(eval_loader))
            with torch.no_grad():
                model.eval()
                traced = torch.jit.trace(model, testbatch['img'].to(device)) #without need to reload the original class definition
