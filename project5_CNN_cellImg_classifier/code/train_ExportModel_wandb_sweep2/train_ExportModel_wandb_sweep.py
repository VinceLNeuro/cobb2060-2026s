...
def run_training():
    ...
    # wandb sweep agent populates wandb.config with sampled hyperparameters
    with wandb.init() as run:
        ...
        full_dataset = ImgDataset(cfg.train_data_dir, augment=False, img_size=cfg.img_size)

        # Stratified train-test 80/20 split
        train_idx, val_idx = make_StratifiedShuffleSplit(full_dataset, val_frac=cfg.val_fraction, random_seed=cfg.random_seed)
        print(f"Split: {len(train_idx)} train / {len(val_idx)} val")
        
        # Turn on the augmentation and ONLY select train data
        train_dataset = ImgDataset(cfg.train_data_dir, augment=True, img_size=cfg.img_size)
        ...
        eval_loader = torch.utils.data.DataLoader(
            eval_subset, batch_size=cfg.batch_size, shuffle=False, # no need to bother
            num_workers=4, pin_memory=True
        )
        
        #### Create an instance of the model ####
        model = MyCNN(n_conv_blocks=cfg.n_conv_blocks, base_ch=cfg.base_ch, fc_size=cfg.fc_size,
                    dropout_p=cfg.dropout_p, img_size=cfg.img_size).to(device)
        
        #### Loss, Optimizer, Scheduler ####
        loss_object = nn.CrossEntropyLoss()
        optimizer = torch.optim.Adam(model.parameters(), lr=cfg.lr, weight_decay=cfg.weight_decay) #regularization in loss
        ...
        for epoch in range(cfg.max_epochs):
            ...
            if val_acc_perEpoch > best_val_acc:
                ...
                export_model = ExportModel(model, img_size=cfg.img_size).to(device)
                ...
                torch.jit.save(traced, cfg.out+f"_{wandb.run.id}.best.pt")
                model.train()
            ...
            #### Log to wandb — metric names must match sweep config ####
            wandb.log({
                "epoch": epoch,
                "train/epoch_loss": train_loss_perEpoch,
                "train/epoch_acc":  train_acc_perEpoch,
                "val/epoch_loss":   val_loss_perEpoch,
                "val/epoch_acc":    val_acc_perEpoch,       # ← this is what the sweep optimizes
